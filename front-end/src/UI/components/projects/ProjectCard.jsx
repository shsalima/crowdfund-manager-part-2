...
export default function ProjectCard({ project }) {
  ...
  const isOpen =
    project.status === "open" && project.currentAmount < project.capital;
  ...
  return (
    <div className="flex gap-6">
      <div className="flex-1 bg-[#111214] border border-zinc-800/50 rounded-2xl p-6 flex flex-col justify-between transition-all hover:border-zinc-700/80">
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {role === "project owner" && (
              <button
                ...
                className="text-zinc-500 hover:text-red-400 p-2 hover:bg-red-500/10 rounded-xl transition-all cursor-pointer"
              >
                <Trash2 size={16} />
              </button>
            )}
        ...
        <Link
          ...
          className="w-full bg-[#16171a] hover:bg-zinc-800 text-white font-medium text-xs py-3 rounded-xl border border-zinc-800/80 flex items-center justify-center gap-2 transition-all cursor-pointer"
        >
      ...
      {/* Invest section - only show for investors on detail page */}
      {role === "investor" && id && <ParticipateSection project={project} />}
    </div>
  );
}

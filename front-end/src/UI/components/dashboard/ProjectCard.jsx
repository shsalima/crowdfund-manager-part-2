  ...
  return (
    <div className="bg-[#111214] border border-zinc-800/80 p-5 rounded-2xl space-y-4 flex flex-col justify-between">
      ...
      <div className="flex justify-between items-center text-xs pt-2 border-t border-zinc-900">
        <div>
          <span className="text-[10px] text-zinc-500 block uppercase font-semibold">Invested</span>
          <span className="font-bold text-zinc-200">${project.raisedAmount?.toLocaleString() || "0"}</span>
        </div>
        <div className="text-right">
          <span className="text-[10px] text-zinc-500 block uppercase font-semibold">Target</span>
          <span className="font-bold text-zinc-200">${project.targetAmount?.toLocaleString() || "0"}</span>
        </div>
      </div>
